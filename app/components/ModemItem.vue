    ...
    <div class="rounded-lg p-4 border-2 transition-all duration-300" :class="modemCardClasses">
        ...
        <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2">
                <div class="w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-sm" :class="providerLogoClasses">
                    {{ providerLogoText }}
                </div>
            ...
            <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" class="sr-only peer" :checked="isModemEnabled" @change="handleToggle" :disabled="isLoading || !hasSimCard" />
                ...
                <div
                    ...
                    :class="[isModemEnabled ? 'bg-green-600 peer-checked:after:translate-x-full' : 'bg-gray-600', !hasSimCard || isLoading ? 'opacity-50 cursor-not-allowed' : '']"></div>
            </label>
        </div>
        ...
        <div class="text-sm space-y-2 mb-3">
            ...
            <!-- Сигнал -->
            <div v-if="isModemEnabled && hasSignal" class="flex justify-between items-center">
                ...
                <div class="flex items-center gap-1">
                    ...
                    <div class="w-16 h-2 bg-gray-600 rounded-full overflow-hidden">
                        <div class="h-full transition-all duration-300" :class="signalStrengthColor" :style="{ width: signalPercentage + '%' }"></div>
                    </div>
                </div>
            </div>
        ...
        <div class="flex gap-2">
            <button
                ...
                :disabled="isLoading"
                class="flex-1 px-3 py-1 bg-blue-600 rounded text-sm hover:bg-blue-700 duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                :class="providerButtonClasses">
                {{ isLoading ? "..." : "Сигнал" }}
            </button>

            <button v-if="!hasSimCard" class="flex-1 px-3 py-1 bg-yellow-600 rounded text-sm transition-colors cursor-not-allowed" disabled>Нет SIM-карты</button>

            <button v-if="!isModemEnabled && hasSimCard" class="flex-1 px-3 py-1 bg-gray-600 rounded text-sm cursor-not-allowed" disabled>Модем выключен</button>
        </div>
    </div>
</template>
...
const modemCardClasses = computed(() => {
    ...
    switch (providerInfo.value.color) {
        case "green":
            return `${base} border-green-500/30 from-green-500/10 to-green-600/5 hover:border-green-400/50`;
        case "red":
            return `${base} border-red-500/30 from-red-500/10 to-red-600/5 hover:border-red-400/50`;
        case "purple":
            return `${base} border-purple-500/30 from-purple-500/10 to-purple-600/5 hover:border-purple-400/50`;
        case "yellow":
            return `${base} border-yellow-500/30 from-yellow-500/10 to-yellow-600/5 hover:border-yellow-400/50`;
        case "blue":
            return `${base} border-blue-500/30 from-blue-500/10 to-blue-600/5 hover:border-blue-400/50`;
        default:
            return `${base} border-gray-500/30 from-gray-500/10 to-gray-600/5`;
    }
});
...
const providerLogoClasses = computed(() => {
    switch (providerInfo.value.color) {
        case "green":
            return "bg-gradient-to-br from-green-600 to-green-700 shadow-lg shadow-green-600/25";
        case "red":
            return "bg-gradient-to-br from-red-600 to-red-700 shadow-lg shadow-red-600/25";
        case "purple":
            return "bg-gradient-to-br from-purple-600 to-purple-700 shadow-lg shadow-purple-600/25";
        case "yellow":
            return "bg-gradient-to-br from-yellow-600 to-yellow-700 shadow-lg shadow-yellow-600/25";
        case "blue":
            return "bg-gradient-to-br from-blue-600 to-blue-700 shadow-lg shadow-blue-600/25";
        default:
...
const statusClasses = computed(() => {
    ...
    const state = props.modem.generic?.state;
    if (!state || state === STATUS.disabled) return "text-gray-400";
    if (state === STATUS.enabled) return "text-green-400";
    if (state === STATUS.connecting || state === STATUS.searching) return "text-blue-400";
    if (state === STATUS.failed) return "text-red-400";
...
const handleToggle = async (event: Event) => {
    ...
    try {
        ...
        props.modem.generic.state = target.checked ? STATUS.enabled : STATUS.disabled;
