      ...
      <div class="bg-[#222228] p-4 rounded-lg text-white cursor-pointer hover:bg-[#2a2930] transition-colors duration-200 border border-gray-700 relative">
        ...
        <div class="flex items-center justify-between mb-3">
          ...
          <div class="flex items-center gap-2">
            <div class="w-2 h-2 rounded-full" :class="hasError ? 'bg-red-500' : 'bg-green-500'" 
                 :title="hasError ? 'Ошибка подключения' : 'Онлайн'"></div>
            <span v-if="lastUpdated" class="text-xs text-gray-400" :title="`Обновлено: ${formatLastUpdated()}`">
              {{ formatLastUpdated() }}
            </span>
            <button v-if="hasError" @click="forceRefresh" class="text-xs text-blue-400 hover:text-blue-300 ml-2">
              Повторить
            </button>
          </div>
        </div>
        ...
        <!-- Сообщение об ошибке -->
        <div v-if="hasError" class="mb-3 p-2 bg-red-500/20 rounded text-xs text-red-300">
          Ошибка загрузки данных. Используются кэшированные данные.
        </div>
